...
(function ($) {
    ...
    var fineBox = {
        ...
        setHash : function () {
            ...
            document.location.hash = fineBox.$overlay.data('fineBox').options.hashArg + '=' + (fineBox.$current.attr('href'));
        },
        ...
        openFromHash : function (hashArg) {
            ...
            var options = fineBox.$overlay.data('fineBox').options,
                uriToOpen,
                elementToOpen,
                hashArgPatt = new RegExp('(#?.*?&?' + hashArg + '=).*'),
                hash = window.location.hash,
                res = hashArgPatt.exec(hash);
            ...
            // Set uri to open
            if (res[1]) {
                uriToOpen = hash.slice(res[1].length);
            } else {
                return;
            }
            ...
            if ((typeof options.hookAlterUriToOpen) === 'function') {
                uriToOpen = options.hookAlterUriToOpen.apply(fineBox, [uriToOpen]);
            }
            ...
            if (elementToOpen.length > 0) {
                ...
                // Apply custom hook
                if ((typeof options.hookOnOpenFromHash) === 'function') {
                    options.hookOnOpenFromHash.apply(fineBox);
                }
            }
        },
