...
(function ($) {
    ...
    Indicator.prototype.add = function () {
        this.$indicator.hide().prependTo(this.$parent).fadeTo('fast', 0.75);
    };
    ...
    var fineBox = {
        ...
        // @url http://jdsharp.us/jQuery/minute/calculate-scrollbar-width.php
        scrollbarWidth : 0,
        getScrollbarWidth : function () {
            if (fineBox.scrollbarWidth === 0) {
                var div, w1, w2;
                div = $('<div style="width:50px;height:50px;overflow:hidden;position:absolute;top:-200px;left:-200px;"><div style="height:100px;"></div>');
                ...
                $(div).remove();
                fineBox.scrollbarWidth = w1 - w2;
            }
            // ;;; log('getScrollbarWidth', fineBox.scrollbarWidth);
            return fineBox.scrollbarWidth;
        },
        ...
        showPager : function () {
            ...
            if ($prevDiv.length === 0) {
                $prevDiv = $('<div />', {
                    'class': data.options.prefix + '-previous ui-state-default ui-corner-all ui-state-hover',
                    'style': 'position: absolute; left: 10px; bottom: 10px; z-index: 10;'
                }).append('<a />').hide().prependTo(fineBox.$container);
            }
            if ($nextDiv.length === 0) {
                $nextDiv = $('<div />', {
                    'class': data.options.prefix + '-next ui-state-default ui-corner-all ui-state-hover',
